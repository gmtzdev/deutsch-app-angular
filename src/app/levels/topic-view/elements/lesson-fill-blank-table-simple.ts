...
function parseStoredRow(rawSentence: string, answers: string[]): StoredRow {
    try {
        const parsed = JSON.parse(rawSentence) as StoredRow;
        ...
        // Fallback for any legacy plain-string row.
    }
            ...
            <div class="le-fbt__table-wrap">
                <table class="le-fbt__table" aria-label="Ejercicio de completar espacios en tabla">
                    ...
                    <tbody>
                        ...
                        <tr [class.le-fbt__row--correct]="rowFeedbacks()[ri] === 'correct'"
                            [class.le-fbt__row--wrong]="rowFeedbacks()[ri] === 'wrong'">
                            <td class="le-fbt__index">{{ ri + 1 }}</td>
                            @for (ci of colIndexes(); track ci) {
                            <td>
                                @if (row.cells[ci]; as cell) {
                                    @if (cell.editable) {
                                    <div class="le-fbt__blank-wrap">
                                        <input class="le-fbt__input" type="text"
                                            [value]="getUserValue(ri, ci)"
                                            [attr.aria-label]="'Celda editable fila ' + (ri + 1) + ', columna ' + (ci + 1)"
                                            [attr.size]="getInputSize(cell.answer)"
                                            autocomplete="off"
                                            autocorrect="off"
                                            spellcheck="false"
                                            (input)="onInput(ri, ci, $any($event.target).value)" />
                                        @if (rowFeedbacks()[ri] === 'wrong' && cell.answer) {
                                        <span class="le-fbt__hint">{{ cell.answer }}</span>
                                        }
                                    </div>
                                    } @else {
                                    <span class="le-fbt__text">{{ cell.text }}</span>
                                    }
                                }
                            </td>
                            }
...
export class LessonFillBlankTable {
    ...
    protected readonly rows = computed(() => this.exercise().rows.map((row) => ({
        id: row.id,
        cells: parseStoredRow(row.sentence, row.answers).cells,
    })));
    protected readonly colCount = computed(() => {
        const max = this.rows().reduce((acc, row) => Math.max(acc, row.cells.length), 0);
        return Math.max(1, max);
    });
    protected readonly colIndexes = computed(() => Array.from({ length: this.colCount() }, (_, i) => i));
    protected readonly headers = computed(() => {
        const cols = this.colCount();
        try {
            const parsed = JSON.parse(this.exercise().style || '') as StoredStyle;
    ...
    constructor() {
        effect(() => {
            ...
            this.userValues.set(rows.map((row) => row.cells.map(() => '')));
            this.rowFeedbacks.set(rows.map(() => null));
            this.result.set(null);
        });
    }

    protected getUserValue(rowIdx: number, colIdx: number): string {
        return this.userValues()[rowIdx]?.[colIdx] ?? '';
    }
    ...
    protected onInput(rowIdx: number, colIdx: number, value: string): void {
        this.userValues.update((vals) => {
            const next = vals.map((row) => [...row]);
    ...
    protected check(): void {
        const rows = this.rows();
        const feedbacks = rows.map((row, ri) => {
            const hasEditable = row.cells.some((cell) => cell.editable);
            if (!hasEditable) return 'correct' as const;
            return row.cells.every((cell, ci) => {
                if (!cell.editable) return true;
                const input = (this.userValues()[ri]?.[ci] ?? '').trim().toLowerCase();
                return input === cell.answer.trim().toLowerCase();
